import {planDetails} from './waiver-ui.js'; import {forecastEvidence} from './forecast-ui.js';
...
export const metricHelp=()=>detail('What do these numbers mean?',`<p><strong>Actual</strong> is the selected week's ESPN score, not a prediction. A dash means unavailable; a real zero stays zero.</p><p><strong>ESPN projection</strong> is ESPN's exact-week forecast. <strong>Estimate</strong> uses that projection unchanged when available; otherwise it uses a qualified learned fallback where available, or the historical baseline. Independent forecasts and their historical intervals are shown separately in player details. The former “median” label meant this central estimate, not a measured statistical median.</p><p><strong>Low / high scenarios</strong> are heuristic ranges, not calibrated probability bounds. <strong>Risk-adjusted lineup score</strong> weights the low, central and high scenarios using your risk settings. It is not a forecast of points. It can change after refresh when ESPN projections, historical spread, your risk weights, or adaptive matchup risk changes. Under balanced weights it can closely match ESPN. Our app does not claim independent projection accuracy: it keeps ESPN unchanged until a separately validated model earns an improvement.</p>`);
...
export function rosterRow(p,teams,prefs,recommended=false) {
  ...
  return `<article class="roster-row"><span class="slot">${esc(slot(recommended?p.assignedSlotId:p.lineupSlotId))}</span><div>${playerIdentity(p,teams)}<small>${esc(p.injuryStatus||'Status unknown')} · ${locked?'Locked / started':p.game?.kickoff?esc(date(p.game.kickoff)):'Kickoff unavailable'}</small></div><div class="score-strip">${metric('Actual',actualNumber(p.actualPoints))}${metric('ESPN proj.',number(p.projectedPoints))}${metric('Risk score',p.hasEstimate?number(p.decisionScore):'—')}</div>${detail('Player details',`${warnings(p.warnings)}<p>${esc(p.estimateSource)} · ${esc(p.matchupLabel)}</p>${forecastEvidence(p)}${preferenceButtons(p,prefs,true)}`)}</article>`;
}

export function waiverCard(p,teams,prefs) {
  return `<article class="card player team-${esc(p.team)}">${playerIdentity(p,teams)}<div class="pill">${esc(p.kind)}${p.kind==='Four-week bridge'?'':` · ${p.mode==='ros'?'Rest of season':p.mode==='bridge'?'Four-week bridge':'This week'}`}</div><h3>${p.drop?`Drop ${esc(p.drop.name)}`:'Use your open roster slot'}</h3><div class="metric-grid">${metric('Lineup gain',signed(p.lineupGain))}${metric(p.mode==='week'?'Starter improvement':'Window improvement',signed(p.waiverValue))}${metric(p.mode==='ros'?'Typical week':'Estimate',number(p.horizonEstimate))}</div>${p.faab?.available?`<p><strong>Value-policy range $${p.faab.low}–$${p.faab.high}</strong> · not a predicted winning bid</p>`:''}${detail('Why this move?',`${(p.reasons||[]).map(r=>`<p>${esc(r)}</p>`).join('')}${planDetails(p)}${forecastEvidence(p)}${warnings(p.warnings)}${p.alternatives?.length?`<p>Other legal drops considered: ${p.alternatives.map(a=>`${esc(a.drop?.name||'open slot')} (${signed(a.lineupGain)} starter points)`).join('; ')}.</p>`:''}`)}${preferenceButtons(p,prefs)}<button class="secondary" data-plan-player="${esc(p.playerId)}">Add to claim plan</button></article>`;
}
