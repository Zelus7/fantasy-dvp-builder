...
export function waiverCard(p,teams,prefs) {
  return `<article class="card player team-${esc(p.team)}">${playerIdentity(p,teams)}<div class="pill">${esc(p.kind)}${p.kind==='Four-week bridge'?'':` · ${p.mode==='ros'?'Rest of season':p.mode==='bridge'?'Four-week bridge':'This week'}`}</div><h3>${p.drop?`Drop ${esc(p.drop.name)}`:'Use your open roster slot'}</h3><div class="metric-grid">${metric('Lineup gain',signed(p.lineupGain))}${metric(p.mode==='week'?'Starter improvement':'Window improvement',signed(p.waiverValue))}${metric(p.mode==='ros'?'Typical week':'Estimate',number(p.horizonEstimate))}</div>${p.faab?.available?`<p><strong>Suggested bid ${p.faab.low}–${p.faab.high}</strong> · cap ${p.faab.max}</p>`:''}${detail('Why this move?',`${(p.reasons||[]).map(r=>`<p>${esc(r)}</p>`).join('')}${planDetails(p)}${forecastEvidence(p)}${warnings(p.warnings)}${p.alternatives?.length?`<p>Other legal drops considered: ${p.alternatives.map(a=>`${esc(a.drop?.name||'open slot')} (${signed(a.lineupGain)} starter points)`).join('; ')}.</p>`:''}`)}${preferenceButtons(p,prefs)}</article>`;
}
