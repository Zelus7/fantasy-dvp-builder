...
import { normalizeWeekList, normalizeWeights,cacheTimestamp,cacheGet } from '../src/db.js';
...
test('D1 cache timestamps sent to browsers are explicit UTC, not device-local time',async()=>{
  assert.equal(cacheTimestamp('2026-09-23 04:00:00'),'2026-09-23T04:00:00.000Z');assert.equal(cacheTimestamp('bad'),null);
  const env={DB:{prepare(){return {bind(){return {async first(){return {payload_json:'{}',updated_at:'2026-09-23 04:00:00',expires_at:'2099-01-01T00:00:00Z'};}}}}}}};
  assert.equal((await cacheGet(env,'fixture')).updatedAt,'2026-09-23T04:00:00.000Z');
});
