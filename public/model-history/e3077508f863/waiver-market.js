...
export function historicalBidContext(position,market={},now=Date.now()){
  ...
  const failures=[...new Map((market.failedOffers||[]).filter(r=>r.position===position&&r.id&&Number.isFinite(Date.parse(r.at))&&Date.parse(r.at)<=now&&now-Date.parse(r.at)<=60*86400000).map(r=>[r.id,r])).values()];
  const failedContext=failures.length?` ${failures.length} processed non-winning ${position} offer(s) also recorded; ${failures.filter(r=>r.status==='unavailable').length} had unavailable targets and ${failures.filter(r=>r.status!=='unavailable').length} failed for another reason. These are not all price losses and are not used as a required winning price.`:'';
  const coverage=` ${market.coverage||'Historical coverage may be incomplete.'}${market.historyStatus&&market.historyStatus!=='fresh'?` Latest history refresh is ${market.historyStatus}; retained confirmed records may omit recent auctions.`:''}`;
  return {sampleSize:n,low:n?amounts[0]:null,high:n?amounts[n-1]:null,median:n?(amounts[Math.floor((n-1)/2)]+amounts[Math.floor(n/2)])/2:null,
    recent:bids.slice(0,8),failedSampleSize:failures.length,recentFailures:failures.slice(0,8),quality:n>=10?'descriptive-history':n?'small-sample':'missing',
    explanation:(n?`${n} verified ${position} winning bid(s) from the last 60 days. Position matching does not make players equal in value. Winning prices do not reveal all losing bids or today's competition; this is context, not a price forecast.`:'No verified recent position-matched winning bids. Missing history is not evidence that the league bids cheaply.')+failedContext+coverage};
}
